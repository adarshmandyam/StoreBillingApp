...
{
    public partial class frmDeliveryNote : BaseClass
    {
        ...
        private string CustomerNamePrint;
        ...
        public frmDeliveryNote()
        {
            ...
            InitializeComponent();
            gvDeliveryNote.CellContentClick += gvDeliveryNote_CellContentClick;
            FillCombo();
            FillGrid();
        }
        ...
        private void FillDataGridForPrint()
        {
            ...
            dtInvoice = deliveryNoteBLL.GetAllDeliveryNote(DeliveryNoteID);
            if (dtInvoice.Rows.Count > 0)
            {

                CustomerNamePrint = Convert.ToString(dtInvoice.Rows[0]["CustomerName"]);
            ...
            dTable = deliveryNoteBLL.GetAllDeliveryNoteItem(DeliveryNoteID, 0);
        ...
        {// Set Company Name, City, Salesperson, Order ID and Order Date
            ...
            CurrentX = leftMargin;
            CurrentY = CurrentY + 8;
            FieldValue = "Customer Name: " + CustomerNamePrint;
        ...
        {// Set Invoice Table:
            ...
            // Set Invoice Table:
            CurrentY = CurrentY + InvoiceFontHeight + 8;
            dtDeliveryNoteItem = deliveryNoteBLL.GetAllDeliveryNoteItem(DeliveryNoteID,0);
            for (int i = 0; i < dtDeliveryNoteItem.Rows.Count; i++)
            {
                FieldValue = dtDeliveryNoteItem.Rows[i]["ProductDesc"].ToString();
                ...
                FieldValue = dtDeliveryNoteItem.Rows[i]["Quantity"].ToString();

                g.DrawString(FieldValue, InvoiceFont, BlackBrush, xQuantity, CurrentY);
                FieldValue = String.Format("{0:0.00}", dtDeliveryNoteItem.Rows[i]["RatePerUnit"]);
                g.DrawString(FieldValue, InvoiceFont, BlackBrush, xUnitPrice, CurrentY);
                FieldValue = dtDeliveryNoteItem.Rows[i]["Quantity"].ToString();
                g.DrawString(FieldValue, InvoiceFont, BlackBrush, xQuantity, CurrentY);
                FieldValue = dtDeliveryNoteItem.Rows[i]["DiscountAmount"].ToString();
                g.DrawString(FieldValue, InvoiceFont, BlackBrush, xDiscount, CurrentY);

                FieldValue = dtDeliveryNoteItem.Rows[i]["TaxAmount"].ToString();
                g.DrawString(FieldValue, InvoiceFont, BlackBrush, xTaxAmt, CurrentY);

                Amount = Convert.ToDecimal(dtDeliveryNoteItem.Rows[i]["TotalAmount"]);
        ...
        private void frmDeliveryNote_Load(object sender, EventArgs e)
        {
            ...
            dtEstimatedDate.Format = DateTimePickerFormat.Custom;
            // Display the date as "Mon 27 Feb 2012".  
            dtDeliveryDate.CustomFormat = ConfigurationManager.AppSettings["DateFormat"];
            dtEstimatedDate.CustomFormat = ConfigurationManager.AppSettings["DateFormat"];
        }

        private void gvDeliveryNote_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            this.dataGridView2.DataSource = null;
            var senderGrid = (DataGridView)sender;

            var aa = gvDeliveryNote.CurrentCell.RowIndex;
            if (senderGrid.Columns[e.ColumnIndex].DisplayIndex == 8)
            {
                DeliveryNoteID = Convert.ToInt32(gvDeliveryNote.Rows[aa].Cells[0].Value);

                DataTable dtDeliveryNote = new DataTable();
                dtDeliveryNote = deliveryNoteBLL.GetAllDeliveryNote(DeliveryNoteID);
                if (dtDeliveryNote != null)
                {
                    if (dtDeliveryNote.Rows.Count > 0)
                    {
                        txtName.Text = Convert.ToString(dtDeliveryNote.Rows[0]["CustomerName"]);
                        txtRemarks.Text = Convert.ToString(dtDeliveryNote.Rows[0]["Remarks"]);

                        cmbInvoice.SelectedValue = Convert.ToString(dtDeliveryNote.Rows[0]["InvoiceID"]);
                        cmbDeliveryMode.SelectedValue = Convert.ToString(dtDeliveryNote.Rows[0]["DeliveryModeID"]);                        

                        dtDeliveryDate.Text = Convert.ToString(dtDeliveryNote.Rows[0]["DeliveryDate"]);
                        dtEstimatedDate.Text = Convert.ToString(dtDeliveryNote.Rows[0]["EstimatedDeliveryDate"]);
                        
                    }

                    dtDeliveryNoteItem = new DataTable();
                    dtDeliveryNoteItem = deliveryNoteBLL.GetAllDeliveryNoteItem(DeliveryNoteID, 0);
                    dataGridView2.AutoGenerateColumns = false;
                    dataGridView2.DataSource = dtDeliveryNoteItem;
                    if (dtDeliveryNoteItem.Rows.Count > 0)
                    {
                    ...
                    this.gvDeliveryNote.AllowUserToAddRows = false;
                    GetProduct();
                    GetUnit();
                }
            }
            else if ((senderGrid.Columns[e.ColumnIndex] is DataGridViewButtonColumn) && (senderGrid.Columns[e.ColumnIndex].DisplayIndex == 9 && e.RowIndex >= 0) && (!(String.IsNullOrEmpty(gvDeliveryNote.Rows[aa].Cells[0].Value.ToString()))))
            {
                DeliveryNoteID = Convert.ToInt32(gvDeliveryNote.Rows[aa].Cells[0].Value);
                bool check = deliveryNoteBLL.DeleteDeliveryNote(DeliveryNoteID, 1);
                if (check == true)
                    MessageBox.Show("Selected Delivery Note Deleted Successfully.");
                else
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            //if (validation() == true)
            {
                ...
                DataTable dtDeliveryNote = new DataTable();
                dtDeliveryNote.TableName = "DeliveryNote";
                dtDeliveryNote.Columns.Add("CustomerName", typeof(String));
                dtDeliveryNote.Columns.Add("InvoiceID", typeof(Int32));
                dtDeliveryNote.Columns.Add("Invoice", typeof(String));
                dtDeliveryNote.Columns.Add("DeliveryModeID", typeof(Int32));
                dtDeliveryNote.Columns.Add("DeliveryMode", typeof(String));
                dtDeliveryNote.Columns.Add("DeliveryDate", typeof(String));
                dtDeliveryNote.Columns.Add("EstimatedDeliveryDate", typeof(String));                
                dtDeliveryNote.Columns.Add("Remarks", typeof(String));

                DataRow rowDeliveryNote = dtDeliveryNote.NewRow();
                rowDeliveryNote["CustomerName"] = txtName.Text.Trim();
                rowDeliveryNote["InvoiceID"] = cmbInvoice.SelectedValue;
                rowDeliveryNote["Invoice"] = "Cash";
                rowDeliveryNote["DeliveryModeID"] = cmbDeliveryMode.SelectedValue;
                rowDeliveryNote["DeliveryMode"] = "Self";
                rowDeliveryNote["DeliveryDate"] = Convert.ToDateTime(dtDeliveryDate.Value).ToString("MM-dd-yy");
                rowDeliveryNote["EstimatedDeliveryDate"] = Convert.ToDateTime(dtEstimatedDate.Value).ToString("MM-dd-yy");
                rowDeliveryNote["Remarks"] = txtRemarks.Text.Trim();               
                dtDeliveryNote.Rows.Add(rowDeliveryNote);


                DataTable dt1 = new DataTable();
                dt1.TableName = "DeliveryNoteDetails";
                dt1.Columns.Add("DeliveryNoteItemID", typeof(Int32));
                dt1.Columns.Add("DeliveryNoteID", typeof(Int32));
                ...
                {
                    row = dt1.NewRow();
                    row["DeliveryNoteItemID"] = dataGridView2.Rows[i].Cells["DeliveryNoteItemID"].Value.ToString() == "" ? "0" : dataGridView2.Rows[i].Cells["DeliveryNoteItemID"].Value;
                    row["DeliveryNoteID"] = DeliveryNoteID;
                ...
                dtDeliveryNote.WriteXml(swSQL2);
                swSQL2.Dispose();

                deliveryNoteBLL.DeliveryNoteSave(DeliveryNoteID, sbSQL2.ToString(), sbSQL1.ToString(), UserId);
        ...
        private void btnAddRow_Click(object sender, EventArgs e)
        {
            ...
            if (dtDeliveryNoteItem == null)
            {
               
                dtItem = new DataTable();
                dtItem.TableName = "InvoiceDetails";
                dtItem.Columns.Add("DeliveryNoteItemID", typeof(Int32));
                dtItem.Columns.Add("DeliveryNoteID", typeof(Int32));
        ...
        {
            DataTable dt = new DataTable();
            dt = deliveryNoteBLL.GetAllDeliveryNote(0);
            GridBind(dt);
        }

        private void GridBind(DataTable dt)
        {
            gvDeliveryNote.AutoGenerateColumns = false;
            dt.Columns.Add("Edit");
            gvDeliveryNote.DataSource = dt;
            this.gvDeliveryNote.AllowUserToAddRows = false;
        }
        ...
        {
            DataTable dt = new DataTable();
            dt = deliveryNoteBLL.GetAllDeliveryMode();
            cmbDeliveryMode.DataSource = dt;
            dt.Rows.Add(new object[] { 0, "Select" });
            dt.DefaultView.Sort = "DeliveryModeID";
            cmbDeliveryMode.ValueMember = "DeliveryModeID";
            cmbDeliveryMode.DisplayMember = "DeliveryMode";
            cmbDeliveryMode.SelectedValue = 0;
        }
        private void GetInvoice()
        {
            DataTable dt = new DataTable();
            dt = deliveryNoteBLL.GetAllInvoice();
            cmbInvoice.DataSource = dt;
            dt.Rows.Add(new object[] { 0, "Select" });
            dt.DefaultView.Sort = "InvoiceID";
            cmbInvoice.ValueMember = "InvoiceID";
            cmbInvoice.DisplayMember = "CustomerName";
            cmbInvoice.SelectedValue = 0;
        }
        private void GetProduct()
        {
            DataTable dt = new DataTable();
            dt = deliveryNoteBLL.GetProduct();
            dt.Rows.Add(new object[] { 0, "Select" });
        ...
        private void ClearFields()
        {
            ...
            dtEstimatedDate.Value = DateTime.Now;            
        }
